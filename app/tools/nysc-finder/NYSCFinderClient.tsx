...
export function NYSCFinderClient() {
  ...
  const fetchNYSCJobs = async () => {
    try {
      ...
      let filtered = (allJobs || []).filter((job: any) =>
        (job.role_category || '').toLowerCase().includes('nysc') ||
        (job.title || '').toLowerCase().includes('nysc')
      );
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className={`px-4 py-2 border rounded-lg flex items-center gap-2 text-sm ${
              filtersOpen ? 'bg-blue-50 border-blue-300 text-blue-700' : ''
            }`}
          >
            <Filter size={16} />
            Filters
